...
// [[=salt::table_constraint("FOREIGN KEY (a, b) REFERENCES t(a, b)")]] —
// a table-level constraint, verbatim, appended after the columns (and the
// composite PRIMARY KEY, when there is one). This is the escape hatch for
// what column annotations cannot say: composite foreign keys, multi-column
// UNIQUE. Trusted developer text like check/sql_default; may appear more
// than once, emitted in declaration order.
struct table_constraint {
  char text[192] = {};
  unsigned size = 0;
  consteval table_constraint(const char* s) {
    for (; s[size]; ++size) {
      if (size >= sizeof(text)) throw "saltherring: table_constraint too long";
      text[size] = s[size];
    }
  }
...
enum class errc : std::uint8_t {
  ...
  exec,                // statement execution failed
  constraint,          // a UNIQUE / FOREIGN KEY / NOT NULL / CHECK refusal —
                       // distinguishable because callers build idioms on it
                       // (insert-or-conflict, allocate-retry loops)
...
struct dialect {
  ...
  bool fold_upper = false;          // quote identifiers UPPERCASED (Oracle:
                                    // unquoted names in tails fold to upper,
                                    // so this keeps tails portable)
};
...
// LONGTEXT/LONGBLOB: MariaDB's plain TEXT and BLOB cap at 64 KiB and fail
// with "Data too long" beyond it; keyed text columns use VARCHAR(255) anyway.
inline constexpr dialect mariadb_dialect{
    "mariadb", placeholder_style::question, '`',
    "BIGINT", "DOUBLE", "LONGTEXT", "VARCHAR(255)", "LONGBLOB", "BOOLEAN",
    "BIGINT PRIMARY KEY AUTO_INCREMENT", false};

// Oracle 23ai (CREATE TABLE IF NOT EXISTS needs 23c+ — the gvenzl/oracle-free
// container qualifies). NUMBER(1) for bool keeps the driver off OCI's
// 23ai-only BOOLEAN type; VARCHAR2(4000) is Oracle's standard limit (CLOB
// columns are future work). Two Oracle semantics the driver surfaces rather
// than hides: '' IS NULL, and DDL commits implicitly (as on MariaDB).
inline constexpr dialect oracle_dialect{
    "oracle", placeholder_style::colon_n, '"',
    "NUMBER(19)", "BINARY_DOUBLE", "VARCHAR2(4000)", {}, "BLOB", "NUMBER(1)",
    "NUMBER(19) GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY", true,
    /*fold_upper=*/true};
...
consteval auto build_columns() {
  ...
  // Composite keys are legal (multi-tenant tables key on (tenant, id)), but
  // an auto-assigned column inside one is not a thing any engine agrees on.
  std::size_t pks = 0;
  bool auto_in_pk = false;
  for (const column_info& c : out) {
    pks += c.is_pk;
    if (c.is_pk && c.auto_inc) auto_in_pk = true;
  }
  if (pks > 1 && auto_in_pk)
    throw "saltherring: auto_pk cannot be part of a composite primary key";
  return out;
}
...
// Every [[=salt::table_constraint]] on T, declaration order, in static
// storage. string_view is not structural, so the array holds pointer+length
// pairs into define_static_string storage.
struct tc_text {
  const char* p;
  std::size_t n;
  constexpr std::string_view str() const { return {p, n}; }
};

template <typename T>
consteval auto build_table_constraints() {
  std::vector<tc_text> out;
  for (auto a :
       std::meta::annotations_of_with_type(^^T, ^^struct table_constraint)) {
    std::string_view s = std::define_static_string(
        std::meta::extract<struct table_constraint>(a).str());
    out.push_back({s.data(), s.size()});
  }
  return std::define_static_array(out);
}

template <typename T>
inline constexpr auto table_constraints = build_table_constraints<T>();
...
struct statement {
  ...
  // Rows this statement changed. Valid once step() has returned false (the
  // statement ran to completion); statement-scoped ON PURPOSE — a
  // connection-global counter is a read-after-write race the moment two
  // statements share the connection.
  virtual result<std::int64_t> affected() = 0;
};
...
inline void quoted(std::string& out, std::string_view id, const dialect& d) {
  out += d.quote;
  if (d.fold_upper)
    for (char c : id) out += (c >= 'a' && c <= 'z') ? char(c - 32) : c;
  else
    out += id;
  out += d.quote;
}
...
inline void placeholder(std::string& out, const dialect& d, int n) {
  if (d.placeholders == placeholder_style::dollar_n) {
    out += '$';
    out += std::to_string(n);
  } else if (d.placeholders == placeholder_style::colon_n) {
    out += ':';
    out += std::to_string(n);
  } else {
    out += '?';
...
inline std::string begin_sql(const dialect& d, tx_options o) {
  ...
  if (d.name == "oracle") {
    // No BEGIN keyword (that opens a PL/SQL block); SET TRANSACTION starts
    // one. Oracle has READ COMMITTED and SERIALIZABLE only — repeatable_read
    // maps up to SERIALIZABLE.
    if (o.read_only) return "SET TRANSACTION READ ONLY";
    if (o.iso == isolation::read_committed)
      return "SET TRANSACTION ISOLATION LEVEL READ COMMITTED";
    if (o.iso == isolation::repeatable_read || o.iso == isolation::serializable)
...
std::string create_table_sql(const dialect& d) {
  ...
  // per-column PRIMARY KEY twice is invalid SQL on every engine.
  constexpr std::size_t pks = detail::pk_count<T>();
  ...
  for (const column_info& c : detail::columns<T>) {
    ...
    if (c.auto_inc) {
      ...
      if (c.is_pk && pks == 1) out += " PRIMARY KEY";
      else if (c.is_pk || !c.nullable) out += " NOT NULL";
    }
  ...
  if constexpr (pks > 1) {
    ...
    for (const column_info& c : detail::columns<T>) {
      if (!c.is_pk) continue;
      if (!std::exchange(first_pk, false)) out += ", ";
      detail::quoted(out, c.name, d);
    }
    out += ')';
  }
  for (const detail::tc_text& tc : detail::table_constraints<T>) {
    out += ", ";
    out += tc.str();
  }
  out += ')';
...
class db {
 public:
  ...
  // exec that answers "how many rows did that change?" — the UPDATE/DELETE
  // companion. An UPDATE that matched nothing is 0, not an error: whether
  // 0 means not_found, conflict, or fine is the caller's domain knowledge.
  template <typename... Args>
  result<std::int64_t> execute(sql q, const Args&... args) {
    return execute_raw(q.text, args...);
  }
  template <typename... Args>
  result<std::int64_t> execute(unchecked_sql q, const Args&... args) {
    return execute_raw(q.text, args...);
  }
 ...
 private:
  ...
  template <typename... Args>
  result<std::int64_t> execute_raw(std::string_view text, const Args&... args) {
    contract_assert(open());
    if (!open()) return fail(errc::closed, "db is not open");
    auto adapted = detail::adapt_placeholders(text, dial());
    if (!adapted) return std::unexpected(adapted.error());
    auto st = prepare_bound(*adapted, args...);
    if (!st) return std::unexpected(st.error());
    // Run to completion — affected() is defined at done, and an UPDATE ...
    // RETURNING (should anyone write one) must not stop at the first row.
    for (;;) {
      auto more = (*st)->step();
...
struct migrate_options {
  ...
  // Concurrency seam: several processes may migrate the same database at
  // startup (a compose stack starting its daemons together). `lock` is taken
  // once, before the history is read, and `unlock` runs on every exit path
  // after a successful lock — Postgres deployments pass
  // pg_advisory_lock/pg_advisory_unlock here, SQLite deployments pass
  // nothing and rely on file locking. The callables receive the db so they
  // can exec; they must not migrate.
  std::function<result<void>(db&)> lock;
  std::function<void(db&)> unlock;
};
...
inline result<migrate_report> migrate(db& d, std::span<const migration> list,
    ...
    pre(d.open()) {
  // The lock is held across the WHOLE run — history read included — so two
  // concurrent migrators serialize before either can decide anything.
  struct unlock_guard {
    db* d;
    const std::function<void(db&)>* fn;
    ~unlock_guard() {
      if (fn && *fn) (*fn)(*d);
    }
  } guard{&d, nullptr};
  if (opts.lock) {
    if (auto l = opts.lock(d); !l) return std::unexpected(l.error());
    guard.fn = &opts.unlock;
  }
